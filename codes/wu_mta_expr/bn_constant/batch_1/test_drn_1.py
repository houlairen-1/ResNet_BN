...
fileList1 = []
for (dirpath_train_cover,dirnames1,filenames1) in os.walk(path_train_cover):  #0~5000 for training  5001~10000 for testing
    fileList1 = filenames1

fileList2 = []
for (dirpath_test_cover,dirnames2,filenames2) in os.walk(path_test_cover):  #0~5000 for training  5001~10000 for testing
...
def model(x):
    ...
    with tf.variable_scope("n3") as scope:
        n3 =  non_bottleneck(d2,256,1)
        d3 = dimension_increase(n3,[3,3,256,512],1)

    with tf.variable_scope("n4") as scope:
        n4 =  non_bottleneck(d3,512,1)
        avgpool = avg_pool(n4,7,4,'SAME',1)
            
    with tf.variable_scope('fully_connecting_1') as scope:
        w6 = weight_variable([8192,1000],6)
...
def process():
    ...
    with tf.Session() as sess:
        saver.restore(sess,"/home/liugq/Workspace/ResNet_BN/data/models/2018.01/DRN_resnet_1_180000.ckpt")
        test_writer = tf.summary.FileWriter("/home/liugq/Workspace/ResNet_BN/data/logs/2018.01/drn_1/test/batch_1/bn_constant")
        cover_count = 0
        stego_count = 0
        ...
        while test_count<40000:
            if test_count%2==0:
                imc=ndimage.imread(path_test_cover+'/'+fileList2[test_count%20000])
                data_y[0,0] = 0
                data_y[0,1] = 1
            else:
                imc=ndimage.imread(path_test_stego+'/'+fileList2[test_count%20000])
                data_y[0,0] = 1
                data_y[0,1] = 0
