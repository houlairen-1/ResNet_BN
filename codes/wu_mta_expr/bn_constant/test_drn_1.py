...
def process():
    ...
    with tf.Session() as sess:
        ...
        test_writer = tf.summary.FileWriter("/home/liugq/Workspace/ResNet_BN/data/logs/2018.01/drn_1/test_vary")
        cover_count = 0
        stego_count = 0
        ...
        while test_count<20000:
            for j in range(BATCH_SIZE):
                if j%2==0:
                    imc=ndimage.imread(path_test_cover+'/'+fileList2[test_count])
                    data_y[j,0] = 0
                    data_y[j,1] = 1
                else:
                    imc=ndimage.imread(path_test_stego+'/'+fileList2[test_count])
                    data_y[j,0] = 1
                    data_y[j,1] = 0
