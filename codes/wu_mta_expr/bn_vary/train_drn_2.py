...
def process():
    ...
    with tf.Session() as sess:
        ...
        train_writer = tf.summary.FileWriter("/home/liugq/Workspace/ResNet_BN/data/logs/2018.01/drn_2/train",sess.graph)
        test_writer = tf.summary.FileWriter("/home/liugq/Workspace/ResNet_BN/data/logs/2018.01/drn_2/test")
        summary1 = tf.Summary()
        summary2 = tf.Summary()
        cover_count = 0
        stego_count = 0
        for i in range(1,NUM_ITER+1):
            ...
            if i%NUM_SHOWTRAIN==0:  
                ...
                print 'DRN resnet_shi result'
                print 'epoch:', i
                print 'loss:', c
                print 'accuracy:', temp
                print ' '
            if i%(30000)==0:
                saver.save(sess,'../../data/models/2018.01/DRN_resnet_2_'+str(i)+'.ckpt')
            if i%NUM_SHOWTEST==0:
                ...
                while test_count<20000:
                    ...
                    #Data.iloc[test_count*2-10:test_count*2,0:2] = pred[:,:]
                    result2 = np.insert(result2,0,temp2)
                    if test_count % 1000==0:
                        print 'temp test:',temp2
