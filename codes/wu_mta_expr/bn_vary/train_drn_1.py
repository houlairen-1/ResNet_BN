...
def process():
    ...
    with tf.Session() as sess:
        ...
        train_writer = tf.summary.FileWriter("/home/liugq/Workspace/ResNet_BN/data/logs/2018.01/drn_1/train",sess.graph)
        test_writer = tf.summary.FileWriter("/home/liugq/Workspace/ResNet_BN/data/logs/2018.01/drn_1/test")
        summary1 = tf.Summary()
        cover_count = 0
        stego_count = 0
        for i in range(1,NUM_ITER+1):
            ...
            summary1,pre,_,c,temp = sess.run([merged,y_,opt,loss,accuracy],feed_dict={x:data_x,y:data_y,is_train:True})
            ...
            if i%NUM_SHOWTRAIN==0: 
                ...
                print ' '
            if i%(NUM_SHOWTEST)==0:
                saver.save(sess,'../../data/models/2018.01/DRN_resnet_1_'+str(i)+'.ckpt')
            ################################ TEST ########################################################################
            if i%NUM_SHOWTEST==0:
                ...
                summary2 = tf.Summary(value=[tf.Summary.Value(tag="test_acc", simple_value=np.mean(result2)),])
